...
class MainActivity : AppCompatActivity(), OnMapReadyCallback, GoogleMap.OnMyLocationButtonClickListener, GoogleMap.OnMyLocationClickListener {
    ...
    private fun createMarker() {
        val coordenadas =
            LatLng(-8.063169, -34.871139) // Posição em Coordenadas latitude e longitude qualquer
        val marker: MarkerOptions = MarkerOptions().position(coordenadas)
            .title("Praça do Marco zero - Recife") // Marca: val coordenadas e nomeia o lugar com: .title
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        toggle_button.setOnCheckedChangeListener { buttonView, isChecked ->
            ...
            if(isChecked)
            {
                val pairCalAvg = endRun()
                textView1.text = "${pairCalAvg.first} cal"
                textView2.text = "${pairCalAvg.second} km/h"
            }
            else
            {
                textView1.text = ""
